	  ...
	  return 0;
   	...
   	function feedSetup(feedItems,item) {
        ...
        for (i = 0; i < 3; i++) {
            ...
            if(item == "Retargeting"){
                ...
                if(widget.defaults.adSize == "300x600" && i==0){
					var country = dynamicData['Nike2_Title'][0]["Country"];
                    //headlineText('BRIEF2_Frame4','BRIEF2_Frame5','BRIEF2_Frame6');
                    $('.t1').each(function() {
                        this.innerHTML = dynamicData['Nike2_Title'][0]["BRIEF2_Frame4"];
                        shrinkr(this);
						 if(country == "spain_spanish" || country == "spain_catalan"){
							 this.style.cssText += "line-height:18px";
						 }
				...
				if(widget.defaults.adSize == "160x600"){
					if(country == "spain_spanish" || country == "spain_catalan"){
						document.getElementById("t1").style.cssText += "line-height:26px";
						document.getElementById("t2").style.cssText += "line-height:26px";
						document.getElementById("t3").style.cssText += "line-height:26px";
					}
				}
				if(widget.defaults.adSize == "120x600"){
					if(country == "spain_spanish" || country == "spain_catalan"){
						document.getElementById("t1").style.cssText += "line-height:23px";
						document.getElementById("t2").style.cssText += "line-height:23px";
						document.getElementById("t3").style.cssText += "line-height:23px";
					}
				}
		...
		if(widget.defaults.adSize == "300x600"){
			...
			if(widget.defaults.adSize == "300x250"){
				if(navigator.userAgent.indexOf("Mac") > -1){
					if(navigator.userAgent.indexOf('Safari') != -1){
						document.getElementById("cta_txt").style.cssText += "font-size:14px;line-height:19px";
					}
				}
				if(ap_DataSignal7 == "mexico_spanish" || ap_DataSignal7 == "spain_spanish" || ap_DataSignal7 == "chile_spanish" || ap_DataSignal7 == "puerto rico_spanish"){
					if(navigator.userAgent.indexOf("Mac") > -1){
						document.getElementById("cta_txt").style.cssText += "font-size:11px;line-height:19px";
					}
				}
			}
		}
    ...
    (function() {
        ...
        this.generateGallery = function(result,creativeVariationReporting,customWidgetReporting) {
			...
			for (var key in result) {
				...
				if(result[key]['model_type'] == "Footwear"){
					modeltypeReporting.push("FTW");
				}else{
					modeltypeReporting.push("APP");
				}
				cat_Reporting.push(result[key]['Category']);
				gen_Reporting.push(result[key]['Gender']);
			}
            ...
            //reportingupdate
				...
				var assetVariantReportValues = strategy+"|"+geoValue+"|"+langValue+"|"+asv_values["dspValue"]+"|"+asv_values["deviceValue"]+"|"+asv_values["creativeType"]+"|"+asv_values["creativeSize"]+"|"+asv_values["videoValue"]+"|"+cat_Reporting.join(",")+"|"+gen_Reporting.join(",")+"|"+asv_values["dvLineitemID"]+"|"+asv_values["dvAuctionID"]+"|"+asv_values["dvCreativeID"]+"|"+asv_values["dvSiteID"]+"|"+asv_values["cmPID"]+"|"+asv_values["cmAdID"]+"|"+asv_values["cmRenderID"]+"|"+asv_values["cmCreativeID"]+"|"+asv_values["cmSiteID"]+"|"+asv_values["otherDSPLineID"]+"|"+asv_values["otherDSPAuctionID"]+"|"+asv_values["otherDSPCreativeID"]+"|"+asv_values["otherDSPRefsiteTag"]+"|"+asv_values["otherDSPAID"]+"|"+productReporting.join(',')+"|"+asv_values["productBGcolors"]+"|"+asv_values["productBGcolorNo"]+"|"+asv_values["dspProductModelType"]+"|"+modeltypeReporting.join(",")+"|"+default_strategy+"|"+asv_values["messageID"]+"|"+asv_values["mindSetValue"];
            ...
            $("#banner").attr("onclick", "").unbind("click").on("click", function(){
                    ...
                    if(SPLASH.app.currentProductURL == SPLASH.app.productArray.length){
						...
						jvxAd.recordEventByName(ap_geoLang+"_"+result[SPLASH.app.productArray.length-1]['ID']+"_Interaction", 1);
						...
						jvxAd.recordAssetVariation("click", assetVariantReportValues);
						jvxAd.recordEventByName(ap_geoLang+"_"+result[SPLASH.app.currentProductURL -1]['ID']+"_Interaction", 1);
                    }
                });
            var report_1 = document.getElementById("report1");
                var att_Val1 = document.createAttribute("data-report1");
                att_Val1.value = ap_geoLang+"_"+result[0]['ID'];
			 ...
			 var report_2 = document.getElementById("report2");
                ...
                att_Val2.value = ap_geoLang+"_"+result[1]['ID'];
			 ...
			 var report_3 = document.getElementById("report3");
                ...
                att_Val3.value = ap_geoLang+"_"+result[2]['ID'];
		...
		this.getCatogoryProductsType = function(result) {
            ...
            if (result['g_l_g-c_B2:g_l_g-c_B2'].length >0) { // wips changed
				...
				if(productSet.length >= widget.defaults.numberOfProducts){
				 ...
				 }else if ((ap_geoLang !== "NA_NA")&& (typeof dynamicData != "undefined") && (typeof dynamicData['aid_reports_B2'] != "undefined") && (dynamicData['aid_reports_B2'].length >= 1) && (dynamicData['aid_reports_B2'][0]['Reporting'] != "No Audience ID")) { //Audience catagory Id via DS2 (Feed) - Priority 2 
					console.log('Audience Id available | Priority 2', dynamicData['aid_reports_B2'][0]['Reporting']);
					var dspDS2 = dynamicData['aid_reports_B2'][0]['Reporting'];
					self.validateAudienceId(dspDS2)
				}else if ((self.getDynamicData('tagVarService:tagVarService.DataSignal1')) && (ap_geoLang !== "NA_NA")&& (typeof dynamicData != "undefined") && (typeof dynamicData['lid_report_B2'] != "undefined") && (dynamicData['lid_report_B2'].length >= 1) && (dynamicData['lid_report_B2'][0]['category']!="") && (ap_DataSignal8 == "undefined")&& (ap_DataSignal1 != "${CAMPAIGN_ID}")) {//Prospecting DCM Id via DS1 (Google sheet) - Priority 3
					console.log('Prospecting DCM Id available | Priority 3', self.getDynamicData('tagVarService:tagVarService.DataSignal1'));
					var dspDS1_val = dynamicData['lid_report_B2'][0]['category'].toLowerCase();
        ...
        this.getaudienceSet = function(result) {
            ...
            if (result['g_l_aid_B2:g_l_aid_B2'].length >= widget.defaults.numberOfProducts) {
				...
				var template_value = dynamicData['aid_reports_B2'][0]['Template'];
				if(template_value == "CategoryProspecting"){
					var audienceIdReport = "LineItemIds_";
				}else{
					var audienceIdReport = "AudienceId_";
				}
				var aid_value = dynamicData['aid_reports_B2'][0]['Reporting'];
		...
		this.validateConverterId = function(converterid) {
			...
			var catDBConverter = ap_geoLang + "_" + converterid;
        ...
        this.init = function() {
			...
			console.log(ap_bestsellerID,ap_DataSignal8,ap_DataSignal1,"ds6Val");
			...
			if ((ap_DataSignal8 != "undefined") && (ap_geoLang !== "NA_NA") && (typeof dynamicData != "undefined") && (typeof dynamicData['lid_report_B2'] != "undefined") && (dynamicData['lid_report_B2'].length >= 1) && (dynamicData['lid_report_B2'][0]['category']!="")) {
				console.log('Converter Id available | Priority 0', ap_DataSignal8);
				var dspDS8 = ap_DataSignal8;
				var dspDS8_cat = dynamicData['lid_report_B2'][0]['category'].toLowerCase();
				console.log(dspDS8,"dspDS8");
                self.validateConverterId(dspDS8_cat);
			}else if ((self.isRetargeting()) && (ap_geoLang !== "NA_NA") && (self.getDynamicData('cookieData:cookieData.jvxdynsl_group1'))) { //Retargeting - Priority 1 // && (1==2)
                console.log('isRetargeting | Priority 1 ');
                var cookieValG1 = self.getDynamicData('cookieData:cookieData.jvxdynsl_group1');
                self.validateRetargetingProducts(cookieValG1)
